...
contract Multicall3 is Ownable {
    ...
    function aggregate(
        Call[] calldata calls
    ) public payable onlyOwner returns (uint256 blockNumber, bytes[] memory returnData) {
    ...
    function tryBlockAndAggregate(
        bool requireSuccess,
        Call[] calldata calls
    ) public payable onlyOwner returns (uint256 blockNumber, bytes32 blockHash, Result[] memory returnData) {
        blockNumber = block.number;
