...
contract MusicWithMinting is Context, MinterRole {
    ...
    uint public totalSupply = 2000000000e18; // 2 billion MUSIC
    ...
    function transferFrom(address src, address dst, uint rawAmount) external payable returns (bool) {
        ...
        if (spender != src && spenderAllowance != type(uint96).max) {
            uint96 newAllowance = sub96(
                spenderAllowance,
                amount,
                'Music::transferFrom: transfer amount exceeds spender allowance'
            );
